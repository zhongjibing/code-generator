...
import com.icezhg.codegenerator.utils.FieldTypeUtil;
...
public class MySqlGenerator implements Generator{
    ...
    public List<Table> getTables() {
        return tableService.find(getSchemas(), TableType.BASE_TABLE.getName(), null);
    }

    @Override
    public List<Column> getTableColumns(Table table) {
        return columnService.find(table.getTableSchema(), table.getTableName());
    }

    @Override
    public void generate(String schemaName) throws IOException {
        for (Table tb : getTables()) {
            List<Column> columns = getTableColumns(tb);
            generate(tb, columns);
        }
    }

    private void generate(Table table, List<Column> columns) throws IOException {
        generateDomain(table, columns);
//        generateMapper(table, columns);
//        generateService(table, columns);
//        generateController(table, columns);

    }

    private void generateDomain(Table table, List<Column> columns) throws IOException {
        Builder builder = TypeSpec.classBuilder(NameUtil.className(table.getTableName())).addModifiers(Modifier.PUBLIC);

        for (Column column : columns) {
            Object type = FieldTypeUtil.getType(column);
            String fieldName = NameUtil.fieldName(column.getColumnName());
        ...
        JavaFile javaFile = JavaFile.builder(getConfig().getBasePackage() + ".domain", builder.build()).build();
        javaFile.writeTo(new File("E:\\tmp\\srcource"));
    }


    public void generate() throws IOException {
        generate(getConfig().getSchema());
    }
